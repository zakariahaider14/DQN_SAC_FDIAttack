...
I_BASE_HV_AC = S_BASE / (np.sqrt(3) * V_BASE_HV_AC)  # HV AC base current
I_BASE_LV_AC = S_BASE / (np.sqrt(3) * V_BASE_LV_AC)  # LV AC base current
...
def physics_loss(model, t, Y_bus_tf, bus_data, attack_actions, defend_actions):
    ...
    try:
        ...
        t = tf.reshape(tf.convert_to_tensor(t, dtype=tf.float32), [-1, 1])
        attack_actions = tf.reshape(tf.convert_to_tensor(attack_actions, dtype=tf.float32), [-1, NUM_EVCS * 2])
        defend_actions = tf.reshape(tf.convert_to_tensor(defend_actions, dtype=tf.float32), [-1, NUM_EVCS * 2])

        # Initialize loss components
        evcs_loss = []
        attack_loss = tf.constant(0.0, dtype=tf.float32)
        voltage_violation_loss = tf.constant(0.0, dtype=tf.float32)
        voltage_regulation_loss = tf.constant(0.0, dtype=tf.float32)
        modulation_index_vout = tf.constant(0.0, dtype=tf.float32)
        
        # Extract attack and defense actions with proper reshaping
        ...
        with tf.GradientTape(persistent=True) as main_tape:
            ...
            V = safe_op(tf.exp(predictions[:, :NUM_BUSES]))  # [batch_size, NUM_BUSES]
            theta = safe_op(tf.math.atan(predictions[:, NUM_BUSES:2*NUM_BUSES]))  # [batch_size, NUM_BUSES]# [batch_size, NUM_EVCS * 18]
            ...
            for i, bus in enumerate(EVCS_BUSES):
                try:
                    ...
                    evcs = tf.reshape(evcs_vars[:, i*18:(i+1)*18], [-1, 18])
                    
                    # Split with explicit shapes
                    splits = tf.split(evcs, 18, axis=1)
                    v_ac, i_ac, v_dc, i_dc, v_out, i_out, i_L1, i_L2, v_c, soc, delta, omega, phi_d, phi_q, gamma_d, gamma_q, i_d, i_q = [
                        tf.ensure_shape(split, [tf.shape(evcs)[0], 1]) for split in splits
                    ]
                    ...
                    modulation_index_vout = safe_op(tf.clip_by_value(WAC_KP_VOUT * wac_error_vout + WAC_KI_VOUT * wac_integral_vout, 0, 1))

                    v_out = safe_op(v_dc * (modulation_index_vout + modulation_index_system))
                    
                    v_out_loss = safe_op(tf.reduce_mean(tf.square(v_out_ref - v_out)))
                    ...
                    voltage_deviation = tf.abs(v_out_ref- v_out)
                    ...
                    upper_violation = tf.maximum(0.0, v_out - MAX_VOLTAGE_PU)
                    lower_violation = tf.maximum(0.0, MIN_VOLTAGE_PU - v_out)
                    ...
                    v_out_regulation_loss = safe_op(tf.reduce_mean(
                        tf.square(tf.maximum(0.0, 0.95 - v_out) + 
                    ...
                    evcs_losses = [
                        ...
                        safe_op(tf.reduce_mean(tf.square(di_d_dt_loss))),
                        safe_op(tf.reduce_mean(tf.square(di_q_dt_loss))),
                        safe_op(tf.reduce_mean(tf.square(di_L1_dt_loss))),
                        safe_op(tf.reduce_mean(tf.square(di_L2_dt_loss))),
                        safe_op(tf.reduce_mean(tf.square(dv_c_dt_loss))),
                        safe_op(tf.reduce_mean(tf.square(v_out - v_out_attacked))),
                        safe_op(tf.reduce_mean(tf.square(v_out_constraint))),
