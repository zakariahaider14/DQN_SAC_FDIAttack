...
# EVCS Parameters
EVCS_CAPACITY = 50e3 / S_BASE  # 80 kW in per-unit
EVCS_EFFICIENCY = 0.99
EVCS_VOLTAGE = V_BASE_DC / V_BASE_LV  # In p.u.

GRID_VOLTAGE = 4160  # 12.6 kV
...
EVCS_OUTER_KP = 5 #0.5 and 0.3 was original value
EVCS_OUTER_KI = 0.01
...
LCL_L1 = 0.002 / Z_BASE_LV  # Convert to p.u.
LCL_L2 = 0.002 / Z_BASE_LV  # Convert to p.u.
LCL_CF = 0.002 * S_BASE / (V_BASE_LV**2)  # Convert to p.u.
R = 0.1 / Z_BASE_LV  # Convert to p.u.
C_dc = 0.01 * S_BASE / (V_BASE_LV**2)  # Convert to p.u.


L_dc = 0.001 / Z_BASE_LV  # Convert to p.u.
v_battery = 480 / V_BASE_DC  # Convert to p.u.
...
line_data = [
    (1, 2, 0.1233, 0.4127), (2, 3, 0.0140, 0.6050), (3, 4, 0.7463, 1.2050),
    (4, 5, 0.6984, 0.6084), (5, 6, 1.9831, 1.7276), (6, 7, 0.9053, 0.7886),
    (7, 8, 2.0552, 2.1640), (8, 9, 0.0010, 0.0010), (9, 10, 0.3463, 0.7500),
    (10, 11, 0.7486, 0.7114), (11, 12, 0.2898, 0.1845), (12, 13, 0.2682, 0.1385)
]


bus_data = np.array([
    [1, 0, 0, 0], [2, 184, 116, 0], [3, 98, 98, 0], [4, 47, 34, 0],
    [5, 176, 125, 0], [6, 139, 17, 0], [7, 0, 0, 0], [8, 0, 0, 0],
    [9, 0, 0, 0], [10, 0, 0, 0], [11, 88, 60, 0], [12, 122, 74, 0],
    [13, 63, 34, 0]
])


bus_data[:, 1:3] = bus_data[:, 1:3]*10**3 / S_BASE
EVCS_CAPACITY = 50e3 / S_BASE    # Convert kW to per-unit
...
LEARNING_RATE_ATTACK = 1e-5  # Separate learning rate for attack optimization
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        for i, bus in enumerate(EVCS_BUSES):
            ...
            evcs_loss.extend([
                ...
                dv_dc_dt_loss, v_out_loss, v_out_constraint, di_out_dt_loss,
...
class ModelTrainer:
    def __init__(self, num_evcs, initial_learning_rate=1e-6):
        ...
        #     tf.random.uniform([num_evcs], minval=-0.05, maxval=0.05, dtype=tf.float32),
        #     trainable=True,
        #     name="attack_vector"
        # )
    ...
    def plot_attack_history(self):
        ...
        for i in range(NUM_EVCS):
            ax1.plot(time_steps, attacks[:, i], label=f'EVCS {i+1} at Bus {EVCS_BUSES[i]}')
        ...
        for i in range(NUM_EVCS):
            ...
            ax2.plot(time_steps, attacked_v[:, i], '-', label=f'Attacked V{i+1}')
        ...
        for i in range(NUM_EVCS):
            print(f"EVCS {i+1} at Bus {EVCS_BUSES[i]}:")
...
if __name__ == '__main__':
    try:
        ...
        plt.savefig('voltage_without_attack.pdf')
        ...
        if best_attack is not None:
            ...
            plt.savefig('voltage_with_attack.pdf')
